...
- Ceph supports encryption format LUKS1 and LUKS2. To use the LUKS format, it will start by formatting the image
```
$ rbd encryption format {pool-name}/{image-name} {luks1|luks2} {passphrase-file} [–cipher-alg {aes-128 | aes-256}]
```
- LUKS Headers are generated by format operation and are written to the beginning of image.

- In order to mount encrypted LUKS image use the following 
```
$ rbd -p {pool-name} device map -t nbd -o encryption-format={luks1|luks2},encryption-passphrase-file={passphrase-file}
```
- As the encryption format and encryption load operations are CPU-intensive, and may take a few seconds to complete. (Depends on the server capacity)
