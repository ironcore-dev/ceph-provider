...
// EventStoreOptions defines options to initialize the event store
...
func NewEventStore(log logr.Logger, opts EventStoreOptions) *Store {
	return &Store{
		...
		eventResyncInterval: opts.EventResyncInterval,
		head:                0,
		count:               0,
		log:                 log,
	}
...
func (es *Store) Eventf(log logr.Logger, apiMetadata api.Metadata, eventType, reason, messageFormat string, args ...any) {
	metadata, err := api.GetObjectMetadataFromObjectID(apiMetadata)
	if err != nil {
